...
/** TPS display mode */
type TpsDisplayMode = "tps" | "ttft" | "stats" | "full";
...
interface HudConfig {
	...
	/** TPS display mode. Default: "tps". */
	tpsDisplay?: TpsDisplayMode;
	/** TPS sliding window in ms. Default: 1000. */
	tpsWindow?: number;
	/** TPS speed thresholds. Default: slow=0, medium=15, fast=30, blazing=45. */
	tpsThresholds?: { slow?: number; medium?: number; fast?: number; blazing?: number };
	/** TPS speed colors (hex). */
	tpsColors?: { slow?: string; medium?: string; fast?: string; blazing?: string };
	/** TPS end-of-stream behavior: "average" (total avg) or "last" (last window). Default: "average". */
	tpsEndBehavior?: "average" | "last";
	/** TTFT display mode: "always" (show always) or "brief" (show for first 3s then hide). Default: "brief". */
	ttftDisplay?: "always" | "brief";
	/**
...
export default function (pi: ExtensionAPI) {
	...
	// ---- TPS tracking (sliding window) ----
	let tpsTimestamps: number[] = [];
	let tpsTokenCount = 0;
	let tpsStartTime = 0;
	let tpsDisplay = "";
	let ttftMs: number | null = null;
	let userMsgTimestamp = 0;
	...
	function hexColor(text: string, hex: string): string {
	...
	function getTpsColor(tps: number, cfg: HudConfig): string {
		const t = cfg.tpsThresholds || {};
		const c = cfg.tpsColors || {};
		if (tps >= (t.blazing ?? 45)) return c.blazing ?? "#44ddff";
		if (tps >= (t.fast ?? 30)) return c.fast ?? "#00ff88";
		if (tps >= (t.medium ?? 15)) return c.medium ?? "#ffaa00";
		return c.slow ?? "#ff4444";
	}
	...
	function updateTpsDisplay(cfg: HudConfig) {
		...
		while (tpsTimestamps.length > 0 && tpsTimestamps[0] < windowStart) {
			tpsTimestamps.shift();
		}
		const windowTokens = tpsTimestamps.length;
		const windowElapsed = (now - Math.max(tpsStartTime, windowStart)) / 1000;
		const windowTps = windowElapsed > 0 ? windowTokens / windowElapsed : 0;
		if (isStreaming) lastWindowTps = windowTps;
		const totalElapsed = tpsStartTime > 0 ? (now - tpsStartTime) / 1000 : 0;
		const totalTps = totalElapsed > 0 ? tpsTokenCount / totalElapsed : 0;
		const endBehavior = cfg.tpsEndBehavior ?? "average";
		const tps = isStreaming ? windowTps : (endBehavior === "average" ? totalTps : lastWindowTps);

		const mode = cfg.tpsDisplay ?? "tps";
		const value = String(Math.round(tps));
		const color = getTpsColor(tps, cfg);
		const colored = hexColor(`${value} tok/s`, color);

		const showTtft = ttftMs !== null && !(cfg.ttftDisplay === "brief" && ttftHidden);

		switch (mode) {
			case "ttft":
				tpsDisplay = `⚡ TPS: ${colored}${showTtft ? ` (TTFT: ${ttftMs} ms)` : ""}`;
				break;
			case "stats":
				tpsDisplay = `⚡ TPS: ${colored} (${tpsTokenCount} tok in ${totalElapsed.toFixed(1)}s)`;
				break;
			case "full":
				tpsDisplay = `⚡ TPS: ${colored} (${tpsTokenCount} tok/${totalElapsed.toFixed(1)}s${showTtft ? ` · TTFT: ${ttftMs} ms` : ""})`;
				break;
			default:
				tpsDisplay = `⚡ TPS: ${colored}`;
				break;
		}
	}
		...
		ctx.ui.setFooter((tui, theme, footerData) => {
			...
			return {
				...
				render(width: number): string[] {
					...
					if (isEnabled("tps") && tpsDisplay) {
						line2Items.push({
							key: "tps", defaultLine: 1, order: 5,
							fixedCol: config.placement?.tps?.col,
							render: () => theme.fg("accent", tpsDisplay),
						});
					}
	...
	// ---- Track TPS from streaming events ----
	pi.on("message_start", async (event) => {
		if (event.message?.role === "user") {
			userMsgTimestamp = Date.now();
	...
	pi.on("message_update", async (_event, ctx) => {
		...
		if (!firstTokenReceived) {
			...
			ttftMs = userMsgTimestamp > 0 ? now - userMsgTimestamp : 0;
			tpsStartTime = now;
		}
		if (tpsStartTime === 0) tpsStartTime = now;
		isStreaming = true;
		tpsTimestamps.push(now);
		tpsTokenCount++;
		config = loadConfig(ctx.cwd);
		// Hide TTFT after 3s in brief mode
		if (config.ttftDisplay === "brief" && ttftMs !== null && (now - tpsStartTime) > 3000) {
			ttftHidden = true;
		}
		updateTpsDisplay(config);
	});
	...
	pi.registerCommand("tps", {
		description: "Cycle TPS display mode: tps / ttft / stats / full",
		handler: async (_args, ctx) => {
			const cfg = loadConfig(ctx.cwd);
			const modes: TpsDisplayMode[] = ["tps", "ttft", "stats", "full"];
			const current = cfg.tpsDisplay ?? "tps";
			const idx = modes.indexOf(current);
			const next = modes[(idx + 1) % modes.length];
			const hudCfgPath = join(ctx.cwd, ".pi", "pi-hud.json");
			try {
				const existing = existsSync(hudCfgPath) ? JSON.parse(readFileSync(hudCfgPath, "utf-8")) : {};
				existing.tpsDisplay = next;
				writeFileSync(hudCfgPath, JSON.stringify(existing, null, 2));
				ctx.ui.notify(`TPS mode: ${next}`, "info");
			} catch {
				ctx.ui.notify(`TPS mode: ${next} (not saved)`, "info");
			}
			config.tpsDisplay = next;
		},
	});
}
